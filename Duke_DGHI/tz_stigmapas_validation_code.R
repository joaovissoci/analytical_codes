...
#MAC
data <- read.csv("/Users/Joao/Box/Home Folder jnv4/Data/Global EM/Africa/Tz/BNI/Tz_bnipatients_data.csv", header = TRUE)
...
# 1 factor model - negative factor
cfa_model <- '
...
# Negative_factor =~ alc_treatment_failure+
#                     think_less_treated_person+
#                     less_opinion_trtd_person+
#                     # recover_alcoholic_chldrn+
#                     non_alcoholic_hired
...
#Cronbach's alpha
BNI_onefactormodel_reduced<-with(data,data.frame(
...
psych::alpha(BNI_onefactormodel_reduced,n.iter=1000,check.keys=TRUE)

# Composite reliability
sum(Est$std.all[1:7])^2/(sum(Est$std.all[1:7])^2+
  sum(Est$std.all[43:49]))

#Average Extracted Variance
sum(Est$std.all[1:7]^2)/length(Est$std.all[1:7])
...
BNI_Discrimination =~ alcoholic_close_friend+
                      recovered_alcoholic_teacher+
                      recover_alcoholic_chldrn+
                      recover_alcoholic_hired+
                      non_alcoholic_hired+
