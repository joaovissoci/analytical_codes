...
lapply(c("epicalc", "sem","Hmisc","ggplot2", "psych", "RCurl","irr", "nortest", "moments","GPArotation","Kendall","nFactors","repmis","gdata","vcd","polycor","ROCR","pROC","vcd","psy"), library, character.only=T)
...
data_cluster_police <- read.csv("/Users/joaovissoci/Dropbox/datasets/DGHI/Africa_DGHI/surveyXpolice_validation/POL_RWA_severity.csv", sep=",")

data_cluster_survey <- read.csv("/Users/joaovissoci/Dropbox/datasets/DGHI/Africa_DGHI/surveyXpolice_validation/SURVEY_RWA_severity.csv",sep=",")


#if you are using a file that is local to your computer, then replace path below by path to the data file. command will throw all the data into the templateData object. replace the word template.data by a name that might easier for you to remember and that represents your data
data_cluster_police <- read.csv("/Users/joaovissoci/Dropbox/datasets/DGHI/Africa_DGHI/surveyXpolice_validation/police_SL_1612.csv",sep=",")

data_cluster_survey <- read.csv("/Users/joaovissoci/Dropbox/datasets/DGHI/Africa_DGHI/surveyXpolice_validation/survey_sl_1612.csv",sep=",")
...
##Adjusting NA's
data_cluster_police$RISK<-data_cluster_police$MEANpolice
data_cluster_police$RISK<-car::recode(data_cluster_police$RISK,"NA=0")
##Calculate tertiles
#quantile(data_cluster_police$RISK, 0:4/4)
#data_cluster_police$RISK<-car::recode(data_cluster_police$RISK,"0.0000:1049.274=0;else=1")
data_cluster_police$RISK<-car::recode(data_cluster_police$RISK,"0.0000:349.758000=0;349.758001:1049.274=1;else=2")
#data_cluster_police$RISK<-as.factor(data_cluster_police$RISK)
#Recoding survey data kernel density into low and high risk areas
data_cluster_survey$RISK<-data_cluster_survey$MEANsurvey
data_cluster_survey$RISK<-car::recode(data_cluster_survey$RISK,"NA=0")
data_cluster_survey$RISK<-car::recode(data_cluster_survey$RISK,"0.0000:103.818400=0;103.818401:311.455200=1;else=2")
#data_cluster_survey$RISK<-as.factor(data_cluster_survey$RISK)

agree_data_rw<-data.frame(data_cluster_police$RISK,data_cluster_survey$RISK)
#agree_data_rw<-na.omit(agree_data_rw)
#agree_data_rw$data_cluster_police.RISK<-car::recode(agree_data_rw$data_cluster_police.RISK,"1=0;2=1;3=1")
#agree_data_rw$data_cluster_survey.RISK<-car::recode(agree_data_rw$data_cluster_survey.RISK,"1=0;2=1;3=1")
...
#kendall<-Kendall(agree_data_rw$data_cluster_police.RISK,agree_data_rw$data_cluster_survey.RISK)
...
agree_data_rw<-with(agree_data_rw,data.frame(as.numeric(data_cluster_police.RISK),as.numeric(data_cluster_survey.RISK)))
...
#quantile(data_cluster_police$RISK, 0:4/4)
#data_cluster_police$RISK<-car::recode(data_cluster_police$RISK,"0.0000:1049.274=0;else=1")
data_cluster_police$RISK<-car::recode(data_cluster_police$RISK,"0.0000:67.823400=0;67.823401:203.470200=1;else=2")
#data_cluster_police$RISK<-as.factor(data_cluster_police$RISK)
#Recoding survey data kernel density into low and high risk areas
data_cluster_survey$RISK<-data_cluster_survey$MEANsurvey
data_cluster_survey$RISK<-car::recode(data_cluster_survey$RISK,"NA=0")
data_cluster_survey$RISK<-car::recode(data_cluster_survey$RISK,"0.0000:87.445400=0;87.445401:262.336200=1;else=2")
#data_cluster_survey$RISK<-as.factor(data_cluster_survey$RISK)

agree_data_sl<-data.frame(data_cluster_police$RISK,data_cluster_survey$RISK)
#agree_data_sl<-na.omit(agree_data_sl)
#agree_data_sl$data_cluster_police.RISK<-car::recode(agree_data_sl$data_cluster_police.RISK,"1=0;2=1;3=1")
#agree_data_sl$data_cluster_survey.RISK<-car::recode(agree_data_sl$data_cluster_survey.RISK,"1=0;2=1;3=1")
#Executing agreement nalysis
...
#kendall<-Kendall(agree_data_sl$data_cluster_police.RISK,agree_data_sl$data_cluster_survey.RISK)
...
agree_data_sl<-with(agree_data_sl,data.frame(as.numeric(data_cluster_police.RISK),as.numeric(data_cluster_survey.RISK)))
...
tiff("/Users/joaovissoci/Desktop/figure2A.tiff", width = 700, height = 500,compression = 'lzw')
#par(mfrow=c(1,2))
agree_plot_rw<-agreementplot(obj1_rw$table, main = "Rwanda",xlab_rot=0, ylab_rot=90,xlab_just="center", ylab_just="center", xlab="Survey",ylab="Police")
dev.off()

tiff("/Users/joaovissoci/Desktop/figure2B.tiff", width = 700, height = 500,compression = 'lzw')
...
#CI and Plotting
tiff("/Users/joaovissoci/Desktop/figure3.tiff", width = 500, height = 500,compression = 'lzw')
roc1 <- roc(agree_data_rw[,2],
            agree_data_rw[,1], percent=TRUE,
            # arguments for auc
            #partial.auc=c(100, 90), partial.auc.correct=TRUE,
