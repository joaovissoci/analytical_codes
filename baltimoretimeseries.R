...
library(tseries)
library(forecast)
setwd("D:\\Acessorias\\João Ricardo Vissoci")


data=read.table("/Users/joaovissoci/Desktop/eniuce.csv",sep=",",header=T)
head(data)

timeseries_data <- ts(data$prev_crashes_month, #time_series_month$impaired, 
                      start=c(2009,1),frequency=12)

#investigating the series
plot(timeseries_data)
acf(timeseries_data) #autocorrelacao
pacf(timeseries_data) #autocorrelacao parcial
...
#ARIMA Sasonal
#P=1, d=0 , q=2
fit<-arima(timeseries_data, order=c(1,0,2),
	seasonal=list(order=c(0,0,1)), xreg = intervention)
fit
acf(fit$residuals)
pacf(fit$residuals)

#testa a significancia da correlacao dos residuos
