...
data_plot1<-read.csv("/Users/Joao/Box Sync/Home Folder jnv4/Data/DGNN/Ug_neuropatientsphysicalactivity/ug_neurophysicalactivity2_data.csv")
...
# low_intensity
table<-with(data,table(low_intensity_recoded))
table
prop.table(table)
...
# minimal_activity_recoded
table<-with(data,table(moderate_intensity_recoded))
table
prop.table(table)
...
# minimal_activity_recoded
table<-with(data,table(high_intensity_recoded))
table
prop.table(table)
...
# not_observed
table<-with(data,table(no_movement))
table
prop.table(table)
table<-with(data,table(no_movement,surgery_recoded))
table
prop.table(table,2)
table<-with(data,table(no_movement,gcs_observation_cat))
table
prop.table(table,2)
table<-with(data,table(no_movement,current_unit))
table
prop.table(table,2)
table<-with(data,table(no_movement,bed))
table
prop.table(table,2)
table<-with(data,table(no_movement,patient_room))
table
prop.table(table,2)
table<-with(data,table(no_movement,off_unit))
table
prop.table(table,2)
table<-with(data,table(no_movement,or))
table
prop.table(table,2)
table<-with(data,table(no_movement,alone))
table
prop.table(table,2)
table<-with(data,table(no_movement,family))
table
prop.table(table,2)
table<-with(data,table(no_movement,nursing))
table
prop.table(table,2)
table<-with(data,table(no_movement,neurosurgeon))
table
prop.table(table,2)
table<-with(data,table(no_movement,pt))
table
prop.table(table,2)
table<-with(data,table(no_movement,social_worker))
table
prop.table(table,2)
table<-with(data,table(no_movement,other_medical))
table
prop.table(table,2)


logmodel_minimal<-glm(as.factor(minimal_activity_recoded) ~ 
						surgery_recoded + 
						gcs_observation_cat + 
						as.factor(current_unit) + 
						# bed +
      #                   patient_room + 
      #                   off_unit +
      #                   or +
...
#anova(reglogGEU)
exp(coef(logmodel_gos)) # exponentiated coefficients
exp(confint(logmodel_gos)) # 95% CI for exponentiated coefficients
#predict(model1_death, type="response") # predicted values
#residuals(model1_death, type="deviance") # residuals
# logistic.display(logmodel)

data$moderateplushigh_intensity<-with(data,rowSums(data.frame(minimal_activity,
												   low_intensity)))

data$moderateplushigh_intensity_recoded<-car::recode(data$moderateplushigh_intensity,"
									0='Bno';
									1='Ayes'")

logmodel_high<-glm(as.factor(moderateplushigh_intensity_recoded) ~ 
						surgery_recoded + 
						gcs_observation_cat + 
						as.factor(current_unit) + 
						# bed +
...
tmp1<-data.frame(cbind(exp(coef(logmodel_minimal)),
	exp(confint(logmodel_minimal))))
tmp2<-data.frame(cbind(exp(coef(logmodel_high)),
	exp(confint(logmodel_high))))
tmp<-rbind(tmp1,tmp2)
odds<-tmp[-c(1,14),]
odds[22,3]<-13
names(odds)<-c('OR', 'lower', 'upper')
odds$vars<-rep(c(
			 "Other surgery vs. RTI",
			 "Surgery vs. RTI",
			 "Moderate vs. Mild GCS",
			 "Severe vs. Mild GCS",
			 "HDU vs. General Unit",
			 "Alone",
			 "Family",
...
odds$groups<-rep(c(rep("Dignosis/Surgery",2),
					"GCS",
					"GCS",
					"Unit", 
					rep("Person present",7)),2)
odds$models<-c(rep("Any Activity",12),
			   rep("Moderate to High",12))
...
coord_flip() +
labs(x = 'Predictors of Physical Activity', y = 'OR (CI 95%)') +
theme_bw()


data_plot1$values<-data_plot1[,1]

data_plot1$textsize<-car::recode(data_plot1$values,"0.00:0.200=10;
													0.201:0.400=20;
													0.401:0.600=30;
													0.601:0.800=35;
													0.801:1.00=45")

data_plot1$pointsize<-car::recode(data_plot1$values,"0.00:0.200=10;
													0.201:0.400=20;
													0.401:0.600=30;
													0.601:0.800=35;
													0.801:1.00=45")
...
# library(ggplot2)
ggplot(data_plot1, aes(y=labes, x=classification)) +
  geom_point(aes(size = values*100), 
  			 alpha=0.8, 
  			 color="lightgrey",  
  			 show.legend=FALSE) +
     geom_text(aes(label = values*100,
     				size=textsize/10), 
  			color="black")+
  scale_size(range = c(0,25)) +
  theme_bw() +
  xlab(label="") + 
  ylab(label="Physical activity level") + 
  scale_x_discrete(limits = c("no_minimal",
  							  "low",
  							  "moderate",
  							  "high"),
  				   labels = c("No/Minimal activity",
  							  "Low intensity",
  							  "Moderate intensity",
  							  "High intensity")) + 
  facet_grid(data_plot1$groups ~ .,scales="free", space="free") +
