...
### ZPD highest level on case {.value-box}
...
#getting the highest
highest_zpd <- max(as.numeric(na.omit(as.character(spd_rec))))
...
toplot<-plot_ly(tryout_NAtozero,type = "bar", 
          ...
          y = ~show_tell,
          name='Show and Tell') %>%
      add_trace(y = ~active_help, name = 'Active Help') %>%
      add_trace(y = ~passive_help, name = 'Passive Help') %>%
      add_trace(y = ~no_help, name = 'No Help') %>%
      layout(yaxis = list(title = 'Days until No Help'),
