...
data<-read.csv("/home/joao/Dropbox/datasets/uninga/promudt/humor_biomarkers_promudt.csv",sep=",")
...
########################################################
###### Preparation #######################################################
network_data<-subset(data_imputed,data_imputed$time=='preparation')
network_data<-remove.vars(network_data,c("time"))

cor_matrix<-cor(network_data,method="spearman")
#qsgc<-qsgc$rho

groups<-list(Humor=c(1,2,3,4,5,6),Biomarkers=c(7,8,9,10,11,12))
varLabels<-c("Raiva", "Tensão", "Depressão","Vigor","Fadiga","Confusão","CK","AST","TBARS","TIOIS","CAT","SOD")
...
PcorGRAPH<-qgraph(cor_matrix,layout="spring",vsize=vSize,esize=20,graph="pcor",legend.cex = 0.3,cut = 0.4, maximum = 1, minimum = 0.2, repulsion = 0.8,groups=groups,gray=FALSE,color=c("steelblue","gold"),legend=TRUE, nodeNames=varLabels,labels=varNames)#,layout=graph_layout
...
#### PreCompetition #######################################################
network_data<-subset(data_imputed,data_imputed$time=='Pre-competition')
network_data<-remove.vars(network_data,c("time"))

#Default is to use cor_auto from qgraph
cor_matrix<-cor(network_data,method="spearman")
#qsgc<-qsgc$rho
...
PcorGRAPH<-qgraph(cor_matrix,layout="spring",vsize=vSize,esize=20,graph="pcor",legend.cex = 0.3,cut = 0.6, maximum = 1, minimum = 0.2, repulsion = 0.8,groups=groups,gray=FALSE,color=c("steelblue","gold"),legend=TRUE, nodeNames=varLabels,labels=varNames)#,layout=graph_layout
#Findal PCOR representation
#qsgG2<-qgraph(cor_matrix,layout=Lqsg,vsize=6,esize=20,graph="pcor",legend.cex = 0.3,cut = 0.1, maximum = 1, minimum = 0, esize = 20,vsize = 5, repulsion = 0.8,groups=qsggr,gray=TRUE,color=c("gray80","gray50"),legend=F)#,nodeNames=nomesqsg
#Final GLASSO representation
#qsgG3<-qgraph(cor_matrix,layout=Lqsg,vsize=vSize*3,esize=20,graph="glasso",sampleSize=nrow(network_data),legend.cex = 0.6,cut = 0.1, maximum = 1, minimum = 0, esize = 20,vsize = 5, repulsion = 0.8,groups=qsggr,nodeNames=nomesqsg,color=c("gold","steelblue","red","grey80"),borders = FALSE,labels=varNames)#,gray=T,)#,nodeNames=nomesqsg
...
#### Regeneration #######################################################
network_data<-subset(data_imputed,data_imputed$time=='regeneration')
network_data<-remove.vars(network_data,c("time"))
