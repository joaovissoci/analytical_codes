...
tiff("/Users/jnv4/Desktop/depression_sr_overall_attrition.tiff", 
...
###########################################################
#OVERALL ATTRITION
data_model1<-with(data,data.frame(attrition_cg,
	total_sample_cg,author_year))
data_model1<-na.omit(data_model1[order(data_model1$author_year),])
m3<-metaprop(attrition_cg,total_sample_cg,
	sm="PLN",data=data_model1,studlab=author_year)

tiff("/Users/jnv4/Desktop/depression_sr_overall_attrition_control.tiff", 
	width = 1000, height = 2000,compression = 'lzw')
forest(m3)
dev.off()
funnel(m3)
